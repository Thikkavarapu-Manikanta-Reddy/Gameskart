...
{
   $sl="UPDATE signup SET NAME ='$name', EMAIL ='$email',MOBILEPHONE ='$phone',ADDRESS ='$address',PASSWORD ='$passwor' WHERE NAME ='$row[NAME]'";
   if ($conn->query($sl) === TRUE) {
  
} else {
    echo "Error 404: " . $sl . "<br>" . $conn-> error;
}
   $sq="UPDATE login SET NAME ='$name', EMAIL ='$email',PASSWORD ='$passwor' WHERE NAME ='$row[NAME]'";
   if ($conn->query($sq) === TRUE) {
  
} else {
    echo "Error 404: " . $sq . "<br>" . $conn-> error;
}
}
}
...
<button type="button" class="extra" class="btn btn-info" data-toggle="collapse" data-target="#demo1">CHECKOUT</button>
  <div id="demo1" class="collapse ">
    <h2><strong>Shipping Address&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</strong>
      <a href="address.php">Change Address</a></h2>
<?php
...
{
   $sql = "SELECT NAME,EMAIL,MOBILEPHONE,ADDRESS FROM signup WHERE NAME = '$row[NAME]'";
...
$sql = "SELECT PRODUCT,PRICE,QUANTITY,TOTAL FROM cart ";
